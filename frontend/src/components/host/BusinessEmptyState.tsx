...
export default function BusinessEmptyState({ locale }: BusinessEmptyStateProps) {
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center rounded-3xl border border-dashed border-slate-800 bg-slate-900/30 p-8 text-center sm:p-14 md:p-16">
      ...
      <p className="mt-2 max-w-md text-sm text-slate-400 sm:text-base leading-relaxed">
        {tHost("emptyBusinessesSubtitle")}
      </p>
      <Link
        href={ROUTES.HOST_BUSINESS_NEW(locale)}
        style={{ WebkitTapHighlightColor: "transparent" }}
        className="mt-6 inline-flex min-h-[48px] items-center gap-2 rounded-xl bg-amber-500 px-7 py-3 text-sm font-bold text-slate-950 shadow-lg shadow-amber-500/20 transition-all hover:bg-amber-400 active:scale-95"
      >
